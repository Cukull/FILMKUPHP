...
export default function CafeClient({ menu }: { menu: FnBItem[] }) {
  const [cart, setCart] = useState<{ item: FnBItem, qty: number }[]>([]);
    ...
    <div style={{ display: 'flex', gap: '4rem', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 2, minWidth: '300px' }}>
        ...
        <div className="movie-grid" style={{ padding: 0 }}>
          {menu.length === 0 ? (
            <p style={{ color: 'var(--text-secondary)' }}>Belum ada menu yang tersedia saat ini.</p>
          ) : (
            menu.map(item => (
              <div key={item.id} className="glass" style={{ padding: '1rem', borderRadius: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <img 
                  src={item.imageUrl || `https://via.placeholder.com/150/e50914/FFFFFF?text=${encodeURIComponent(item.name)}`} 
                  alt={item.name} 
